...
namespace INRA.SiriusQualityIrradiance.Strategies
{
	...
	/// <summary>
	///Class CalculateAbsIrradianceSunShadeCanopy
    /// Calculate the absorbed irradiance for the shaded and sunlit leaves of the canopy.
    /// </summary>
	public class CalculateAbsIrradianceSunShadeCanopy : IStrategySiriusQualityIrradiance
	{

	#region Constructor

			public CalculateAbsIrradianceSunShadeCanopy()
			{
	...
	#region Implementation of IAnnotatable
			...
			{
				get { return "Calculate the absorbed irradiance for the shaded and sunlit leaves of the canopy."; }
			}
        ...
        Dictionary<int, double> Ig)
            {
                ...
                Dictionary<int, double> I_shade_absDict = new Dictionary<int, double>();
                    ...
                    double gaiTotal = gaiDict.Sum(x => x.Value.Item1 + x.Value.Item2);

                    double I_cb_c = I_dir * (1.0 - sigma) * (1 - Math.Exp(-k1_dir * gaiTotal));
                    double I_cd_dir = I_dif * (1.0 - rho_dif) * (1 - Math.Exp(-(k_dif + k1_dir) * gaiTotal)) * (k_dif / (k_dif + k1_dir));
                    double I_cs_dir = I_dir * ((1 - rho_dir) * (1 - Math.Exp(-(k_dir + k1_dir) * gaiTotal)) * (k_dir / (k_dir + k1_dir))
                        - (1 - sigma) * (1 - Math.Exp(-2 * k1_dir * gaiTotal)) / 2);

                    // Absorbed irradiance per unit GROUND area
                    double I_sun = (I_cb_c + I_cd_dir + I_cs_dir);
                    //double I_shade = (I_cd_dif + I_cs_dif);
                    double I_shade = Ig[99]- I_sun;


            //***********************************
            I_sun_absDict.Add(99, I_sun);
                    I_shade_absDict.Add(99, I_shade);
