...
namespace INRA.SiriusQualityIrradiance.Strategies
{
	...
	/// <summary>
	///Class CalculateGlobalAbsorbedRadiationHourlyLayers
    /// Calculate global absorbed radiation hourly and leaf layer by leaf layer
    /// </summary>
	public class CalculateGlobalAbsorbedRadiationHourlyLayers : IStrategySiriusQualityIrradiance
	{

	#region Constructor

			public CalculateGlobalAbsorbedRadiationHourlyLayers()
			{
				...
				pd2.PropertyName = "incidentDiffuseIrradiance";
				pd2.PropertyType = (( INRA.SiriusQualityIrradiance.Interfaces.ExogenousVarInfo.incidentDiffuseIrradiance)).ValueType.TypeForCurrentValue;
				pd2.PropertyVarInfo =( INRA.SiriusQualityIrradiance.Interfaces.ExogenousVarInfo.incidentDiffuseIrradiance);
				_inputs0_0.Add(pd2);
				PropertyDescription pd3 = new PropertyDescription();
				pd3.DomainClassType = typeof(INRA.SiriusQualityIrradiance.Interfaces.Exogenous);
				pd3.PropertyName = "incidentDirectIrradiance";
				pd3.PropertyType = (( INRA.SiriusQualityIrradiance.Interfaces.ExogenousVarInfo.incidentDirectIrradiance)).ValueType.TypeForCurrentValue;
				pd3.PropertyVarInfo =( INRA.SiriusQualityIrradiance.Interfaces.ExogenousVarInfo.incidentDirectIrradiance);
				_inputs0_0.Add(pd3);
				...
				pd5.DomainClassType = typeof(INRA.SiriusQualityIrradiance.Interfaces.Rates);
				pd5.PropertyName = "absorbedGlobalIrradianceLayeredHourly";
				pd5.PropertyType =  (( INRA.SiriusQualityIrradiance.Interfaces.RatesVarInfo.absorbedGlobalIrradianceLayeredHourly)).ValueType.TypeForCurrentValue;
				pd5.PropertyVarInfo =(  INRA.SiriusQualityIrradiance.Interfaces.RatesVarInfo.absorbedGlobalIrradianceLayeredHourly);
				_outputs0_0.Add(pd5);
	...
	#region Implementation of IAnnotatable
			...
			{
				get { return "Calculate global absorbed radiation hourly and leaf layer by leaf layer"; }
			}
    ...
    #region Instances of the parameters
			...
			{ 
				get {
						...
						else throw new Exception("Parameter 'Kl' not found (or found null) in strategy 'CalculateGlobalAbsorbedRadiationHourlyLayers'");
				 } set {
							VarInfo vi = _modellingOptionsManager.GetParameterByName("Kl");
							if (vi != null)  vi.CurrentValue=value;
						else throw new Exception("Parameter 'Kl' not found in strategy 'CalculateGlobalAbsorbedRadiationHourlyLayers'");
				}
			}
	...
	#region pre/post conditions management		
			...
			public string TestPostConditions(INRA.SiriusQualityIrradiance.Interfaces.Rates rates,INRA.SiriusQualityIrradiance.Interfaces.Exogenous exogenous,INRA.SiriusQualityIrradiance.Interfaces.States states, string callID)
			{
				try
				{
					...
					if(r4.ApplicableVarInfoValueTypes.Contains( INRA.SiriusQualityIrradiance.Interfaces.RatesVarInfo.absorbedGlobalIrradiance.ValueType)){prc.AddCondition(r4);}
					...
					if(r5.ApplicableVarInfoValueTypes.Contains( INRA.SiriusQualityIrradiance.Interfaces.RatesVarInfo.absorbedGlobalIrradianceLayeredHourly.ValueType)){prc.AddCondition(r5);}
			...
			public string TestPreConditions(INRA.SiriusQualityIrradiance.Interfaces.Rates rates,INRA.SiriusQualityIrradiance.Interfaces.Exogenous exogenous,INRA.SiriusQualityIrradiance.Interfaces.States states, string callID)
			{
				try
				{
					...
					if(r2.ApplicableVarInfoValueTypes.Contains( INRA.SiriusQualityIrradiance.Interfaces.ExogenousVarInfo.incidentDiffuseIrradiance.ValueType)){prc.AddCondition(r2);}
					...
					if(r3.ApplicableVarInfoValueTypes.Contains( INRA.SiriusQualityIrradiance.Interfaces.ExogenousVarInfo.incidentDirectIrradiance.ValueType)){prc.AddCondition(r3);}
	...
	#region Model
            ...
            private Dictionary<int, double> CalculateLayerHourly(
                  double I_dir, double I_dif, double Kl,
                  Dictionary<int, Tuple<double, double>> gaiDict)
            {
                Dictionary<int, double> absDict = new Dictionary<int, double>();
                double absorbedPAR = new double();
                double cumGAI = 0.0;

            for (int layerIndex = gaiDict.Count - 1; layerIndex >= 0; --layerIndex)
            {

                // calculate absorbed PAR on each leaf layer
                absorbedPAR = (I_dir + I_dif) * (Math.Exp(-Kl * cumGAI) - Math.Exp(-Kl * (cumGAI+gaiDict[layerIndex].Item1))); // only lamina absorption is considered
                
                absDict[layerIndex] = absorbedPAR;

                    cumGAI += (gaiDict[layerIndex].Item1 + gaiDict[layerIndex].Item2);
                }
